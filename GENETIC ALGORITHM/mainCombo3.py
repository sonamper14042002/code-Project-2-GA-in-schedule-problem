...
from function import generate_population, fitness_func, selecting_top_two, multi_point_crossover, multi_point_mutation
...
for generation in range(generations):  
    print("\nat generation ", f, ":")
    fitness_bucket = [fitness_func(genome) for genome in society]
    print(fitness_bucket, "---số lượng cá thể cá thể ở thế hệ", f, "là", len(society))
    
    # 1
    parents = selecting_top_two(society)
    dad, mom = parents
    
    print("\nDad:", dad, fitness_func(dad))
    print("\nMom:", mom, fitness_func(mom))

    cross_childs = multi_point_crossover(dad, mom)
    offsrpings_generation = []
    for i in range(len(cross_childs)):
        print("\nchild", i + 1, ":", cross_childs[i], fitness_func(cross_childs[i]))
    if fitness_func(cross_childs[0]) > fitness_func(dad):
        offsrpings_generation.append(cross_childs[0])
    if fitness_func(cross_childs[1]) > fitness_func(mom):
        offsrpings_generation.append(cross_childs[1])
    society.extend(offsrpings_generation)
    
    # 3
    mutated_child_1 = multi_point_mutation(cross_childs[0])
    mutated_child_2 = multi_point_mutation(cross_childs[1])
